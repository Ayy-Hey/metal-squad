...
public class GrenadeSmoke : BaseGrenade
{
	public override void OnInit(PlayerMain _player, bool FlipX, bool hasDamage = true)
	{
		...
		{
			GameManager.Instance.fxManager.CreateGrenadeToxic(this.transform.position).transform.localScale = new Vector3(3f, 3f, 3f);
			GameManager.Instance.fxManager.CreateFire2(this.transform.position);
		}));
	}
	...
	{
		base.OnDisable();
		GameManager.Instance.bombManager.PoolBaseGrenade[3].Store(this);

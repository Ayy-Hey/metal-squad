...
namespace Player
{
	public class Ct9GunPlayer : RamboWeapon
	{
		public override void OnInit(WeaponProfile weaponProfile, PlayerMain inputPlayer)
		{
			base.OnInit(weaponProfile, inputPlayer);
			this.bone1 = this.player._PlayerSpine.FindBone("GunTip_Ct9-1");
			this.bone2 = this.player._PlayerSpine.FindBone("GunTip_Ct9-2");
		...
		public override void OnShootBullet()
		{
			...
			{
				this.player.syncRamboState.SendRpc_CreateBulletCt9(vector2, this.directionGun);
			}
			GameManager.Instance.bulletManager.CreateBulletCt9(this.player, vector2, this.directionGun, true);
		}

...
namespace Player
{
	public class IceGunPlayer : RamboWeapon
	{
		public override void OnInit(WeaponProfile weaponProfile, PlayerMain inputPlayer)
		{
			base.OnInit(weaponProfile, inputPlayer);
			this.bone1 = this.player._PlayerSpine.FindBone("GunTip_Ice-1");
			this.bone2 = this.player._PlayerSpine.FindBone("GunTip_Ice-2");
		...
		public override void OnShootBullet()
		{
			...
			{
				this.player.syncRamboState.SendRpc_CreateICE(vector2, this.directionGun);
			}
			GameManager.Instance.bulletManager.CreateICE(this.player, vector2, this.directionGun, true);
		}

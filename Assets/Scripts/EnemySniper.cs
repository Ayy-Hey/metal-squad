...
public class EnemySniper : BaseEnemy2
{
	...
	private void OnTriggerEnter2D(Collider2D collision)
	{
		...
		{
			if (this._state == EnemySniper.EState.Day1 || this._state == EnemySniper.EState.Parachute)
			{
				if (this.dayDu)
				{
					this.dayDu.Off();
			...
			this._state = EnemySniper.EState.Jump1;
	...
	private void OnCollisionEnter2D(Collision2D collision)
	{
		switch (this._state)
		{
		case EnemySniper.EState.Jump3:
			this.rigidbody2D.velocity = Vector2.zero;
			this.ChangeState();
			break;
		case EnemySniper.EState.Parachute:
		case EnemySniper.EState.Day1:
			if (this.dayDu)
			{
				this.dayDu.Off();
				this.dayDu = null;
			...
			this.ChangeState();
			break;
		case EnemySniper.EState.Walk1:
		case EnemySniper.EState.Walk2:
	...
	public override void Init(EnemyDataInfo enemyDataInfo, Action hideCallback)
	{
		...
		{
			this._state = EnemySniper.EState.Idle;
			this.ChangeState();
		}
		else
		{
			this._isSit = (UnityEngine.Random.Range(0, 2) == 1);
			this._state = EnemySniper.EState.Walk1;
			this.ChangeState();
		}
	}
	...
	public override void OnUpdate(float deltaTime)
	{
		...
		if (this.isAttack && !this.isDie)
		{
			if (!this._isStartAttack)
			{
				...
				if (this._state == EnemySniper.EState.Hit_Ice || flag)
				{
					return;
				}
				this._isStartAttack = true;
				this._isResetBullet = false;
				this._isLockAttack = false;
				base.PlayAnim(1, 1, false);
			}
			else if (this._isResetBullet)
			{
	...
	public override void SetParachuter(float gravity = 0.5f)
	{
		...
		this._state = ((!flag) ? EnemySniper.EState.Parachute : EnemySniper.EState.Day1);
		if (flag)
		{
			this._state = EnemySniper.EState.Day1;
		...
		{
			this._state = EnemySniper.EState.Parachute;
		}
	...
	protected override void UpdateState(float deltaTime)
	{
		...
		if (this.isMove)
		{
			if (base.Flip)
			{
				...
				bool flag = this.pos.x < this._targetX && (this._state == EnemySniper.EState.Walk1 || this._state == EnemySniper.EState.Walk2);
				if (flag)
				{
					this.ChangeState();
				}
			...
			{
				this.pos.x = this.pos.x + this._speed * deltaTime;
				bool flag2 = this.pos.x > this._targetX && (this._state == EnemySniper.EState.Walk1 || this._state == EnemySniper.EState.Walk2);
				if (flag2)
				{
					this.ChangeState();
				}
	...
	{
		base.OnStuckMove();
		if (this._state == EnemySniper.EState.Walk1 || this._state == EnemySniper.EState.Walk2)
		{
			this._state = EnemySniper.EState.Jump1;
			this.isChangeState = false;
		}
	...
	protected override void ChangeState()
	{
		...
		switch (this._state)
		{
		case EnemySniper.EState.Hit_Ice:
		case EnemySniper.EState.Idle:
		case EnemySniper.EState.Idle_Walk:
		case EnemySniper.EState.Day2:
			this.sitCollider.enabled = false;
			this.bodyCollider2D.enabled = true;
			if (!this.cacheEnemyData.ismove)
			{
				this._state = ((!this._isSit) ? EnemySniper.EState.Idle : EnemySniper.EState.Idle_Walk);
			}
			else if (this.canAttack)
			{
				...
				this._state = ((Mathf.Abs(this._targetX - this.pos.x) <= 3f) ? EnemySniper.EState.Walk2 : EnemySniper.EState.Walk1);
			}
			else
			{
				bool flag = false;
				...
				if (flag)
				{
					float num2 = UnityEngine.Random.Range(3f, 4f);
				...
				else
				{
					...
					this._state = EnemySniper.EState.Walk2;
				}
			}
			break;
		case EnemySniper.EState.Jump1:
			this._state = EnemySniper.EState.Jump2;
			break;
		case EnemySniper.EState.Jump2:
			this._state = EnemySniper.EState.Jump3;
			break;
		case EnemySniper.EState.Jump3:
			this.isMove = false;
			this._state = ((UnityEngine.Random.Range(0, 2) != 1) ? EnemySniper.EState.Idle_Walk : EnemySniper.EState.Idle);
			break;
		case EnemySniper.EState.Parachute:
		{
		...
		case EnemySniper.EState.Walk2:
			...
			break;
		case EnemySniper.EState.Day1:
			this._state = EnemySniper.EState.Day2;
			break;
		}
		base.ChangeState();
	}
	...
	{
		if (this._state == EnemySniper.EState.Hit_Ice || this._state == EnemySniper.EState.Parachute)
		{
	...
	{
		base.Hit();
		if (this._state == EnemySniper.EState.Hit_Ice)
		{
			return;
		}
		...
		this._state = EnemySniper.EState.Hit_Ice;
		if (this.isAttack)
		{
	...
	protected override void OnEvent(TrackEntry trackEntry, Spine.Event e)
	{
		...
		if (e.Data.Name.Contains("shoot"))
		{
			...
			BulletEnemy bulletEnemy = GameManager.Instance.bulletManager.CreateBulletEnemy(12, v, worldPosition, this.cacheEnemy.Damage, speed, 0f);
	...
	protected override void OnComplete(TrackEntry trackEntry)
	{
		...
		case "Attack-lendan":
			if (this.isAttack)
			{
				this._isResetBullet = true;
				base.PlayAnim(0, 2, false);
			}
			break;
		case "Attack_Akm":
			this.attackCount++;
			if (this.attackCount > this.modeLv || this._state == EnemySniper.EState.Hit_Ice)
			{
				this.attackCount = 0;
				base.ResetTimeReload();
	...
	private EnemySniper.EState _state;

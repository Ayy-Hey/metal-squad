...
namespace Simplex
{
	public class Noise
	{
		static Noise()
		{
			Noise.perm = new byte[Noise.permOriginal.Length];
			Noise.permOriginal.CopyTo(Noise.perm, 0);
		}
		...
		public static float[] Calc1D(int width, float scale)
		{
			...
			{
				array[i] = Noise.Generate((float)i * scale) * 128f + 128f;
			}
			return array;
		}
		...
		public static float[,] Calc2D(int width, int height, float scale)
		{
			...
			for (int i = 0; i < width; i++)
			{
				...
				{
					array[i, j] = Noise.Generate((float)i * scale, (float)j * scale) * 128f + 128f;
				}
			}
			return array;
		}
		...
		public static float[,,] Calc3D(int width, int height, int length, float scale)
		{
			...
			for (int i = 0; i < width; i++)
			{
				for (int j = 0; j < height; j++)
				{
					...
					{
						array[i, j, k] = Noise.Generate((float)i * scale, (float)j * scale, (float)k * scale) * 128f + 128f;
					}
				}
			}
			return array;
		...
		{
			return Noise.Generate((float)x * scale) * 128f + 128f;
		}

		public static float CalcPixel2D(int x, int y, float scale)
		{
			return Noise.Generate((float)x * scale, (float)y * scale) * 128f + 128f;
		}

		public static float CalcPixel3D(int x, int y, int z, float scale)
		{
			return Noise.Generate((float)x * scale, (float)y * scale, (float)z * scale) * 128f + 128f;
		}
		...
		internal static float Generate(float x, float y)
		{
			...
			int num15 = Noise.Mod(num2, 256);
			int num16 = Noise.Mod(num3, 256);

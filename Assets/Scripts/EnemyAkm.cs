...
public class EnemyAkm : BaseEnemy2
{
	...
	private void OnTriggerEnter2D(Collider2D collision)
	{
		...
		{
			if (this._state == EnemyAkm.EState.Day1 || this._state == EnemyAkm.EState.Parachute)
			{
				if (this.dayDu)
				{
					this.dayDu.Off();
			...
			this._state = EnemyAkm.EState.Jump1;
	...
	private void OnCollisionEnter2D(Collision2D collision)
	{
		switch (this._state)
		{
		case EnemyAkm.EState.Jump3:
			this.rigidbody2D.velocity = Vector2.zero;
			this.ChangeState();
			break;
		case EnemyAkm.EState.Parachute:
		case EnemyAkm.EState.Day1:
			if (this.dayDu)
			{
				this.dayDu.Off();
				this.dayDu = null;
			...
			this.ChangeState();
			break;
		case EnemyAkm.EState.Walk1:
		case EnemyAkm.EState.Walk2:
	...
	public override void Init(EnemyDataInfo enemyDataInfo, Action hideCallback)
	{
		...
		{
			this._state = EnemyAkm.EState.Idle;
			this.ChangeState();
		}
		else
		{
			this._state = EnemyAkm.EState.Walk1;
			this.ChangeState();
		}
	}
	...
	public override void OnUpdate(float deltaTime)
	{
		...
		if (this.isAttack && !this.isDie)
		{
			if (!this._isStartAttack)
			{
				...
				if (this._state == EnemyAkm.EState.Hit_Ice || flag)
				{
					return;
				}
	...
	public override void SetParachuter(float gravity = 0.5f)
	{
		...
		this._state = ((!flag) ? EnemyAkm.EState.Parachute : EnemyAkm.EState.Day1);
		if (flag)
		{
			this._state = EnemyAkm.EState.Day1;
		...
		{
			this._state = EnemyAkm.EState.Parachute;
		}
	...
	protected override void UpdateState(float deltaTime)
	{
		...
		if (this.isMove)
		{
			if (base.Flip)
			{
				...
				bool flag = this.pos.x < this._targetX && (this._state == EnemyAkm.EState.Walk1 || this._state == EnemyAkm.EState.Walk2);
				if (flag)
				{
					this.ChangeState();
				}
			...
			{
				this.pos.x = this.pos.x + this._speed * deltaTime;
				bool flag2 = this.pos.x > this._targetX && (this._state == EnemyAkm.EState.Walk1 || this._state == EnemyAkm.EState.Walk2);
				if (flag2)
				{
					this.ChangeState();
				}
	...
	{
		base.OnStuckMove();
		if (this._state == EnemyAkm.EState.Walk1 || this._state == EnemyAkm.EState.Walk2)
		{
			this._state = EnemyAkm.EState.Jump1;
			this.isChangeState = false;
		}
	...
	protected override void ChangeState()
	{
		...
		switch (this._state)
		{
		case EnemyAkm.EState.Hit_Ice:
		case EnemyAkm.EState.Idle:
		case EnemyAkm.EState.Idle_Walk:
		case EnemyAkm.EState.Day2:
			this.sitCollider.enabled = false;
			this.bodyCollider2D.enabled = true;
			if (!this.cacheEnemyData.ismove)
			{
				this._state = ((this._state != EnemyAkm.EState.Idle) ? EnemyAkm.EState.Idle : EnemyAkm.EState.Idle_Walk);
			}
			else if (this.canAttack)
			{
				...
				this._state = ((Mathf.Abs(this._targetX - this.pos.x) <= 3f) ? EnemyAkm.EState.Walk2 : EnemyAkm.EState.Walk1);
			}
			else
			{
				bool flag = false;
				...
				else
				{
					...
					this._state = EnemyAkm.EState.Walk2;
				}
			}
			break;
		case EnemyAkm.EState.Jump1:
			this._state = EnemyAkm.EState.Jump2;
			break;
		case EnemyAkm.EState.Jump2:
			this._state = EnemyAkm.EState.Jump3;
			break;
		case EnemyAkm.EState.Jump3:
			this.isMove = false;
			this._state = ((UnityEngine.Random.Range(0, 2) != 1) ? EnemyAkm.EState.Idle_Walk : EnemyAkm.EState.Idle);
			break;
		case EnemyAkm.EState.Parachute:
		{
		...
		case EnemyAkm.EState.Walk2:
			...
			break;
		case EnemyAkm.EState.Day1:
			this._state = EnemyAkm.EState.Day2;
			break;
		}
		base.ChangeState();
	}
	...
	{
		if (this._state == EnemyAkm.EState.Hit_Ice || this._state == EnemyAkm.EState.Parachute)
		{
	...
	{
		base.Hit();
		if (this._state == EnemyAkm.EState.Hit_Ice)
		{
			return;
		}
		...
		this._state = EnemyAkm.EState.Hit_Ice;
	...
	protected override void OnEvent(TrackEntry trackEntry, Spine.Event e)
	{
		...
		if (e.Data.Name.Contains("shoot"))
		{
			...
			BulletEnemy bulletEnemy = GameManager.Instance.bulletManager.CreateBulletEnemy(9, v, worldPosition, this.cacheEnemy.Damage, speed, 0f);
	...
	protected override void OnComplete(TrackEntry trackEntry)
	{
		...
		case "Attack_Akm":
			this.attackCount++;
			if (this.attackCount > this.modeLv || this._state == EnemyAkm.EState.Hit_Ice)
			{
				this.attackCount = 0;
				base.ResetTimeReload();
	...
	private EnemyAkm.EState _state;

	...
	public class EnhancedScroller : MonoBehaviour
	{
		...
		public float ScrollSize
		{
			get
			{
				if (this.scrollDirection == EnhancedScroller.ScrollDirectionEnum.Vertical)
				{
					return Mathf.Max(this._container.rect.height - this._scrollRectTransform.rect.height, 0f);
				}
				return Mathf.Max(this._container.rect.width - this._scrollRectTransform.rect.width, 0f);
			}
		}
		...
		public float NormalizedScrollPosition
		{
			get
			{
				...
				return (scrollPosition > 0f) ? (this._scrollPosition / this.ScrollSize) : 0f;
			}
		}
		...
		public EnhancedScrollerCellView GetCellView(EnhancedScrollerCellView cellPrefab)
		{
			...
			if (enhancedScrollerCellView == null)
			{
				...
				if (this.cellViewInstantiated != null)
				{
					this.cellViewInstantiated(this, enhancedScrollerCellView);
				}
			}
			else if (this.cellViewReused != null)
			{
				this.cellViewReused(this, enhancedScrollerCellView);
			}
		...
		public void ReloadData(float scrollPositionFactor = 0f)
		{
			...
			this._scrollPosition = Mathf.Clamp(scrollPositionFactor * this.ScrollSize, 0f, this.GetScrollPositionForCellViewIndex(this._cellViewSizeArray.Count - 1, EnhancedScroller.CellViewPositionEnum.Before));
			if (this.scrollDirection == EnhancedScroller.ScrollDirectionEnum.Vertical)
			{
		...
		public void JumpToDataIndex(int dataIndex, float scrollerOffset = 0f, float cellOffset = 0f, bool useSpacing = true, EnhancedScroller.TweenType tweenType = EnhancedScroller.TweenType.immediate, float tweenTime = 0f, Action jumpComplete = null, EnhancedScroller.LoopJumpDirectionEnum loopJumpDirection = EnhancedScroller.LoopJumpDirectionEnum.Closest)
		{
			...
			if (this.loop)
			{
				float num5 = this.GetScrollPositionForCellViewIndex(dataIndex, EnhancedScroller.CellViewPositionEnum.Before) + num3;
				float num6 = this.GetScrollPositionForCellViewIndex(dataIndex + this.NumberOfCells, EnhancedScroller.CellViewPositionEnum.Before) + num3;
				float num7 = this.GetScrollPositionForCellViewIndex(dataIndex + this.NumberOfCells * 2, EnhancedScroller.CellViewPositionEnum.Before) + num3;
			...
			{
				num4 = this.GetScrollPositionForDataIndex(dataIndex, EnhancedScroller.CellViewPositionEnum.Before) + num3;
			}
			num4 = Mathf.Clamp(num4, 0f, this.GetScrollPositionForCellViewIndex(this._cellViewSizeArray.Count - 1, EnhancedScroller.CellViewPositionEnum.Before));
			if (useSpacing)
			{
				num4 = Mathf.Clamp(num4 - this.spacing, 0f, this.GetScrollPositionForCellViewIndex(this._cellViewSizeArray.Count - 1, EnhancedScroller.CellViewPositionEnum.Before));
			}
			if (num4 == this._scrollPosition)
			{
			...
			base.StartCoroutine(this.TweenPosition(tweenType, tweenTime, this.ScrollPosition, num4, jumpComplete));
		}
		...
		public EnhancedScrollerCellView GetCellViewAtDataIndex(int dataIndex)
		{
			for (int i = 0; i < this._activeCellViews.Count; i++)
			{
				if (this._activeCellViews[i].dataIndex == dataIndex)
				{
					return this._activeCellViews[i];
				}
			}
			return null;
		}
		...
		{
			this._updateSpacing = false;
			this._layoutGroup.spacing = spacing;
		...
		private void _RecycleCell(EnhancedScrollerCellView cellView)
		{
			...
			if (this._recycledCellViewContainer != null)
			{
		...
		private void Update()
		{
			if (this._updateSpacing)
			{
				this._UpdateSpacing(this.spacing);
				this._reloadData = false;
			}
			if (this._reloadData)
			{
		...
		{
			if (this._initialized && this.spacing != this._layoutGroup.spacing)
			{
				this._updateSpacing = true;
			}
		}
		...
		private void _ScrollRect_OnValueChanged(Vector2 val)
		{
			...
			this._scrollPosition = Mathf.Clamp(this._scrollPosition, 0f, this.GetScrollPositionForCellViewIndex(this._cellViewSizeArray.Count - 1, EnhancedScroller.CellViewPositionEnum.Before));

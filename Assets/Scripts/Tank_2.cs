...
public class Tank_2 : BaseEnemy2
{
	...
	public override void Init(EnemyDataInfo enemyDataInfo, Action hideCallback)
	{
		...
		this._idAttack = 0;
		this._countAttack = 0;
		this._state = Tank_2.EState.Idle;
		this.ChangeState();
	}
	...
	protected override void StartState()
	{
		...
		switch (this._state)
		{
		case Tank_2.EState.Aim:
			track = 1;
			base.Flip = (this.pos.x > GameManager.Instance.player.transform.position.x);
			break;
		case Tank_2.EState.Attack1:
		case Tank_2.EState.Idle:
			base.Flip = (this.pos.x > GameManager.Instance.player.transform.position.x);
			break;
		case Tank_2.EState.Walk01:
		case Tank_2.EState.Walk02:
		{
			loop = true;
			this.isMove = true;
	...
	protected override void ChangeState()
	{
		if (this.isAttack)
		{
			...
			if (idAttack != 0)
			{
				...
				{
					this._idAttack = 1;
					this._state = Tank_2.EState.Attack2;
				}
				else
				{
					this._idAttack = 0;
					this._state = Tank_2.EState.Attack1;
				}
			}
			else
			{
				this._idAttack = 2;
				this._state = Tank_2.EState.Aim;
			}
		}
		else
		{
			...
			case Tank_2.EState.Idle:
			{
				...
				if (flag2)
				{
					this._state = ((base.Flip != this.pos.x > x) ? Tank_2.EState.Walk02 : Tank_2.EState.Walk01);
				}
				else
				{
					this._state = Tank_2.EState.Idle;
	...
	protected override void OnComplete(TrackEntry trackEntry)
	{
		...
		if (text != null)
		{
			...
			{
				this._countAttack++;
				bool flag = (this._state != Tank_2.EState.Attack1) ? (this._countAttack > this.modeLv + 1) : (this._countAttack > this.modeLv);
				if (flag)
				{
					this._countAttack = 0;
					base.ResetTimeReload();
					if (this._state == Tank_2.EState.Attack2)
					{
